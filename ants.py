...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def helper(place, min, max, num):
            if place.bees and min<=num<=max:
                return rANTdom_else_none(place.bees)
            elif place.entrance == beehive:
                return None
            else:
                return helper(place.entrance, min, max, num + 1)
        return helper(self.place, self.min_range, self.max_range, 0)
        # END Problem 3 and 4
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        if self.direction == 'backward' and self.backward_steps < 2:
...
def make_scare(action, bee):
    ...
    # BEGIN Problem EC
    def new_action(gamestate):
        if bee.backward_steps < 2:
            bee.direction = "backward"
            action(gamestate)
            bee.direction = "forward"
            bee.backward_steps += 1
        else:
            action(gamestate)
    return new_action
    # END Problem EC
